...
class BookingRepository extends BookingsEntity
{
    ...
    public function findAllByStatusPendingPayment($per_page = 100, $paged = 1): array
    {
        ...
        $query = $this->_wpdb->prepare(
            "SELECT * FROM $this->table_name WHERE status = 'PENDING_PAYMENT' AND expired = FALSE AND (DATE(booking_date) < CURDATE() OR expires_in < NOW()) ORDER BY id DESC LIMIT %d OFFSET %d;",
            $per_page,
            $offset
        );
     ...
     */
    public function findById(string $id, bool $throwable = false): ?BookingModel
    {
        $bookingData = parent::findById($id);
        if(!$bookingData && $throwable) throw new BookingNotFound();
    ...
    public function update($id, $data, $throwable = false)
    {
        ...
        $data['template'] = maybe_serialize($data['template']);
